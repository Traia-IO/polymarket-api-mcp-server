...
# Script to build and run the Polymarket API MCP Server locally in Docker
...
IMAGE_NAME="polymarket-api-mcp-server"
CONTAINER_NAME="polymarket-api-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Polymarket API MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Polymarket API MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Polymarket API MCP Server is running!${NC}"
echo
